...
namespace Yassin
{
	Renderer::Renderer(int width, int height, HWND hWnd, bool fullscreen, float screenNear, float screenFar)
	{
		HRESULT hr;
		...
		DirectX::XMMATRIX projMatrix = DirectX::XMMatrixPerspectiveFovLH(
			DirectX::XMConvertToRadians(fov), aspectRatio, screenNear, screenFar);
		DirectX::XMStoreFloat4x4(&m_ProjectionMatrix, projMatrix);

		DirectX::XMMATRIX orthoMatrix = DirectX::XMMatrixOrthographicLH(width, height, -100.f, 100.f);
		DirectX::XMStoreFloat4x4(&m_OrthoMatrix, orthoMatrix);
